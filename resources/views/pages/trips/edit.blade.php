                        ...
                        <form method="post" action="{{route('trips.update', $trip->id)}}">
                            ...
                            <div class="row g-3 m-3">
                                <div class="col">
                                    ...
                                    <select name="from_location" class="form-select"
                                        ...
                                        <option value="{{$item->name}}" {{ ( $item->name == $trip->from_location ) ? 'selected' : '' }}>{{$item->name}}</option>
                                        @endforeach
                                    </select>
                                    @error('from_location')
                                ...
                                <div class="col">
                                    ...
                                    <select name="to_location" class="form-select" aria-label="Default select example">
                                        <option value=''>Choose location</option>
                                        @foreach($locations as $item)
                                        <option value="{{$item->name}}" {{ ( $item->name == $trip->to_location ) ? 'selected' : '' }} >{{$item->name}}</option>
                                        @endforeach
                                    </select>
                                    @error('to_location')
                                    <div class="alert alert-danger">{{ $message }}</div>
                            ...
                            <div class="row g-3 m-3">
                                ...
                                <div class="col">
                                    ...
                                    <input type="number" min="0" name="price" class="form-control" placeholder="Enter your fare" value="{{$trip->price}}">
                                    @error('price')
                                    <div class="alert alert-danger">{{ $message }}</div>
                                    @enderror
                                </div>
